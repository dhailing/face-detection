...
use anran\Exception\NoFaceException;
use Exception;
...
class FaceDetector
{
    ...
    {
        $color = imagecolorallocate($this->canvas, 255, 255, 255); //White
     ...
     */
    public function cropFaceToJpeg($outFileName = null, $iv = 0)
    {
        if (empty($this->face)) {
            throw new NoFaceException('No face detected');
        }

        $canvas = imagecreatetruecolor($this->face['w'], $this->face['w'] + $iv);
        imagecopy($canvas, $this->canvas, 0, 0, $this->face['x'], $this->face['y'], $this->face['w'], $this->face['w'] + $iv);
